...
modelo nos algoritmos de [ aprendizagem linear
...
learning-better-together-with.html) do TensorFlow, além dos algoritmos de [
XGBoost ](https://xgboost.readthedocs.io/en/latest/tutorials/model.html)
(links em inglês), sem escrever códigos.

Saiba mais sobre o [ treinamento com algoritmos incorporados
...
com o TensorFlow 1.13.1. Consulte a [ lista de versões de ambiente de execução
...
O suporte para treinamento com TPUs na versão 1.9 do ambiente de execução do
Cloud ML Engine foi encerrado em 8 de março de 2019. Veja as versões
...
A versão 1.13 do ambiente de execução do Cloud ML Engine já está disponível
para treinamento e previsão. Essa versão é compatível com o TensorFlow 1.13 e
inclui outros pacotes, que estão na [ lista de versões de ambiente de execução
](https://cloud.google.com/ai-platform/training/docs/runtime-version-
list?hl=pt_br) .
...
platform/training/docs/notebooks/overview?hl=pt_br) está disponível na versão
Beta. O AI Platform Notebooks permite criar e gerenciar instâncias de máquina
virtual (VM) [ pré-empacotadas com o JupyterLab
](https://jupyterlab.readthedocs.io/en/stable/getting_started/overview.html) e
um conjunto de softwares de aprendizado profundo.
...
nova instância do notebook ](https://cloud.google.com/ai-
...
O Cloud TPU já está disponível de modo geral para o treinamento de modelos do
TensorFlow. As unidades de processamento de tensor (TPUs) são aceleradores
desenvolvidos especialmente pelo Google para cargas de trabalho de machine
learning.

Veja [ como usar TPUs para treinar modelos ](https://cloud.google.com/ml-
engine/docs/tensorflow/using-tpus?hl=pt_br) no Cloud ML Engine e [ leia mais
sobre os preços ](https://cloud.google.com/ai-
platform/training/docs/pricing?hl=pt_br) .
...
do Compute Engine. Isso proporciona mais flexibilidade para alocar recursos
computacionais aos jobs de treinamento. Esse recurso está disponível na versão
Beta.

Ao configurar o job com tipos de máquina do Compute Engine, anexe um conjunto
personalizado de GPUs.
...
As CPUs com processador quad-core estão disponíveis em fase Beta para previsão
...
atualizado.
  ...
  * Os seguintes nomes de máquina usados em Alpha estão **suspensos** : ` mls1-highmem-1 ` e ` mls1-highcpu-4 ` . 
  ...
  * Veja o [ preço atualizado ](https://cloud.google.com/ai-platform/training/docs/pricing?hl=pt_br) dos tipos de máquina disponibilizados. 
...
platform/training/docs/pricing?hl=pt_br) e [ como usar GPUs
...
As versões 1.11 e 1.12 do ambiente de execução do Cloud ML Engine já estão
disponíveis para treinamento e previsão. Essas versões são compatíveis com o
TensorFlow 1.11 e 1.12 respectivamente e outros pacotes, conforme consta na [
lista de versões de ambiente de execução ](https://cloud.google.com/ai-
...
de execução mais antiga que inclui o joblib é a 1.4.
...
atualmente compatíveis na [ lista de versões de ambiente de execução
](https://cloud.google.com/ml-engine/docs/tensorflow/runtime-version-
list?hl=pt_br#tpu-support) .
...
A versão 1.11 do ambiente de execução do Cloud ML Engine foi revertida por
causa de [ erros causados por uma incompatibilidade de versão do CuDNN
](https://stackoverflow.com/q/52733440) durante o treinamento de GPU. A
alternativa atual é usar a versão 1.10. Para mais detalhes, consulte a [ lista
de versões de ambiente de execução ](https://cloud.google.com/ai-
platform/training/docs/runtime-version-list?hl=pt_br) .
...
](https://cloud.google.com/ai-platform/training/docs/pricing?hl=pt_br) e [
...
](https://cloud.google.com/ml-engine/docs/tensorflow/using-gpus?hl=pt_br) e os
[ respectivos preços ](https://cloud.google.com/ai-
platform/training/docs/pricing?hl=pt_br) .
...
A tabela a seguir mostra os preços anteriores e o novos:
...
Platform Training, resultando em significativas reduções de preço.
...
disponibilidade do Cloud TPU no Cloud ML Engine. Veja mais detalhes no [ guia
...
A compatibilidade da previsão on-line com o **scikit-learn** e o **XGBoost**
agora está disponível para todos os usuários.

  * Defina ` framework ` em [ ` projects.models.versions.create ` ](https://cloud.google.com/ai-platform/training/docs/reference/rest/v1/projects.models.versions?hl=pt_br) para especificar o framework de machine learning ao criar uma versão do modelo. Os valores válidos são: ` TENSORFLOW ` , ` SCIKIT_LEARN ` e ` XGBOOST ` . ` TENSORFLOW ` é o padrão. Se você especificar ` SCIKIT_LEARN ` ou ` XGBOOST ` , também será preciso definir ` runtimeVersion ` como 1.4 ou superior na versão do modelo. 
...
do modelo ](https://cloud.google.com/ai-
platform/training/docs/reference/rest/v1/projects.models.versions?hl=pt_br) .
Use esses marcadores para organizar os recursos em categorias. Os rótulos
também estão [ disponíveis nas operações ](https://cloud.google.com/ai-
platform/training/docs/reference/rest/v1/projects.operations?hl=pt_br) . Mas,
...
_Informações de contexto:_ o Cloud TPU ficou disponível para as versões 1.6 e
...
para treinamento e previsão. Esta versão é compatível com o TensorFlow 1.8 e
outros pacotes, conforme descrito na [ lista de versões de ambiente de
execução ](https://cloud.google.com/ml-engine/docs/tensorflow/runtime-version-
list?hl=pt_br) .
...
desenvolvidos especialmente pelo Google, que são usadas para acelerar as
cargas de trabalho de machine learning. Veja [ como usar TPUs para treinar
modelos ](https://cloud.google.com/ml-engine/docs/tensorflow/using-
tpus?hl=pt_br) no Cloud ML Engine.
...
. Veja a seguir os valores disponíveis:
  ...
  * ` GRID_SEARCH ` : uma pesquisa de grade simples dentro do espaço viável. Essa opção será útil principalmente se você quiser especificar um número de tentativas maior do que o número de pontos no espaço viável. Nesse caso, se você não especificar uma pesquisa de grade, o algoritmo padrão do Cloud ML Engine poderá gerar sugestões duplicadas. Para usar a pesquisa de grade, todos os parâmetros precisam ser do tipo ` INTEGER ` , ` CATEGORICAL ` ou ` DISCRETE ` . 
...
para previsões on-line. Esse atributo está na _versão Beta_ .
...
O Python 3.5 está disponível para previsão on-line.
  ...
  * Para detalhes de todos os pacotes disponíveis no Cloud ML Engine, consulte a [ lista de versões de ambiente de execução ](https://cloud.google.com/ml-engine/docs/scikit/runtime-version-list?hl=pt_br) . 
...
para treinamento e previsão. Essa versão é compatível com o TensorFlow 1.6 e
outros pacotes, conforme descrito na [ lista de versões de ambiente de
execução ](https://cloud.google.com/ml-engine/docs/tensorflow/runtime-version-
list?hl=pt_br) .
...
[ Lista de versões de ambiente de execução ](https://cloud.google.com/ml-
...
ambiente de execução do Cloud ML Engine para o TensorFlow 1.4. Para mais
informações, consulte a [ lista de versões de ambiente de execução
](https://cloud.google.com/ml-engine/docs/tensorflow/runtime-version-
list?hl=pt_br) .
...
e [ postagem do blog ](https://cloud.google.com/blog/big-
...
platform/training/docs/pricing?hl=pt_br) , [ esta postagem do blog
...
os preços ](https://cloud.google.com/ai-
platform/training/docs/pricing?hl=pt_br) .
...
A geração de registros de auditoria do Cloud ML Engine está agora em fase
...
Os papéis de IAM predefinidos para o Cloud ML Engine estão disponíveis para
uso geral. Consulte [ Controle de acesso ](https://cloud.google.com/ml-
...
As versões anteriores de ambiente de execução com TensorFlow 0.11 e 0.12 não
...
compatibilidade para versões anteriores de ambiente de execução
](https://cloud.google.com/ml-engine/docs/tensorflow/runtime-version-
...
As GPUs estão agora disponíveis na região us-central1. Para ver a lista
completa de regiões compatíveis com GPUs, consulte [ Como usar GPUs para
modelos de treinamento na nuvem ](https://cloud.google.com/ml-
...
ML Engine está disponível para uso geral em modelos de treinamento e de
...
](https://cloud.google.com/terms/launch-stages?hl=pt_br) Beta. No momento, o
uso dela está sujeito à [ política de preços ](https://cloud.google.com/ai-
platform/training/docs/pricing?hl=pt_br) do Cloud ML Engine e à mesma fórmula
de preços aplicada à previsão em lote. Enquanto estiver em Beta, a previsão
on-line não se destinará ao uso em aplicativos críticos.

**CHANGED:**

Os ambientes que o Cloud ML Engine usa para treinar modelos e conseguir
previsões foram definidos como [ versões de ambiente de execução
...
para usar ao realizar o treinamento, definir um recurso de modelo ou solicitar
previsões em lote. Neste momento, as versões de ambiente de execução diferem
principalmente em relação à versão do TensorFlow compatível com cada uma
delas, mas outras diferenças poderão surgir ao longo do tempo. Procure os
detalhes na [ lista de versões de ambiente de execução
](https://cloud.google.com/ml-engine/docs/tensorflow/runtime-version-
list?hl=pt_br) .

**CHANGED:**

Agora é possível executar jobs de previsão em lote com base nos SavedModels do
TensorFlow que estiverem armazenados no Google Cloud Storage e não hospedados
como uma versão de modelo no Cloud ML Engine. Quando você criar o job, use o
URI do SavedModel em vez de fornecer um modelo ou ID de versão.
...
inglês). É possível usar a tecnologia ou ferramenta que você preferir para
fazer o pré-processamento dos dados de entrada. No entanto, recomendamos o `
tf.Transform ` e os serviços disponíveis no Google Cloud Platform, incluindo o
Google Cloud Dataflow, o Google Cloud Dataproc e o Google BigQuery.

##  v1beta1 (29 de setembro de 2016)

**FEATURE:**

A previsão on-line é um recurso Alfa. Embora o AI Platform Training esteja na
fase Beta, a previsão on-line ainda está passando por mudanças significativas
para melhorar o desempenho. Você [ não será cobrado
](https://cloud.google.com/ai-platform/training/docs/pricing?hl=pt_br) pela
previsão on-line enquanto ela for um atributo Alfa.

**FEATURE:**

O pré-processamento e o restante do SDK do Cloud ML Engine são recursos Alfa.
O SDK está em desenvolvimento ativo para melhorar a integração entre o Cloud
ML Engine e o Apache Beam.
