...
漏洞通常有一段时间的保密期，以便受影响的各方有机会解决这些问题。在这种情况下，GKE 的 [ 版本说明
...
**注意** ：如果您在 GKE 上运行多租户工作负载，请特别注意这些公告。这些漏洞更有可能影响多租户工作负载。有关 GKE
...
所有 GKE 用户都受到针对 CVE-2020-8551 的保护，除非不受信任的用户可以从集群的内部网络中发送请求。使用 [ 主授权网络
](https://cloud.google.com/kubernetes-engine/docs/how-to/authorized-
networks?hl=zh_cn) 可针对 CVE-2020-8552 提供额外的缓解。
...
**2020 年 1 月 24 日更新** ：推出修补版本的过程正在进行中，并且将在 2020 年 1 月 25 日完成。
...
**对多数客户而言，无需执行任何额外操作。只有运行 Windows Server 的节点会受影响。**
...
推出修补版本的过程正在进行中，并且将在 2020 年 1 月 24 日完成。

您可以届时将节点升级为 GKE 修补版，也可以随时使用 Windows 更新手动部署最新的 Windows 补丁程序。
...
辅助信息文件中存在的一个安全问题，此问题会影响到 [ 容器存储接口 (CSI) 驱动程序 ](https://kubernetes-
csi.github.io/docs/drivers.html) 中捆绑的辅助信息文件的大多数版本。如需了解详情，请参阅 [ Kubernetes 披露信息
...
csi/external-resizer) 辅助信息文件中的一个漏洞，可能允许未经授权的卷数据访问或更改。这会影响到 [ CSI 驱动程序
](https://kubernetes-csi.github.io/docs/drivers.html) 内捆绑的辅助信息文件的大多数版本。
...
Intel 披露了多个 CVE，这些 CVE 可能允许推测执行与微架构状态交互，从而导致数据泄露。如需了解详情，请参阅 [ Intel 披露信息
...
在使用 N2、M2 或 C2 节点，否则无需采取进一步措施。 ** N1 节点上的 GKE 实例无需采取新的措施。
...
：此 CVE 也称为 TSX 异步中止 (TAA)。TAA 使用被 [ 微架构数据抽样 (MDS)
](https://cloud.google.com/kubernetes-engine/docs/security-
bulletins?hl=zh_cn#may-14-2019) 所利用的相同微架构数据结构，提供了另一条数据渗漏途径。

[ CVE-2018-12207 ](https://cve.mitre.org/cgi-bin/cvename.cgi?name=2018-12207)
这是一种影响虚拟机主机的拒绝服务攻击 (DoS) 漏洞，允许恶意客机引发未受保护的主机崩溃。此 CVE 也称为“页面大小更改时机器检查错误”。此问题不会影响
GKE。
...
(DoS) 漏洞。在 GKE 中，这些漏洞可能允许用户编写恶意请求，过量占用 Kubernetes API 服务器的 CPU
...
在包含此漏洞缓解措施的最新补丁程序版本发布后，建议您尽快将集群升级到该补丁程序版本。我们预计这些补丁程序版本将根据 [ 发布时间表
](https://cloud.google.com/kubernetes-engine/docs/release-
notes?hl=zh_cn#september_16_2019) ，随下个 GKE 版本一起在所有地区推出。
...
2019 年 5 月 31 日  的公告已更新，添加了关于修复安全公告内记录的漏洞的信息。

##  2019 年 8 月 22 日

2019 年 8 月 5 日  的公告已更新。先前发布的一则公告中记录的漏洞的修复已 [ 发布
](https://cloud.google.com/kubernetes-engine/docs/release-
notes?hl=zh_cn#august_22_2019) 。

##  2019 年 8 月 8 日

2019 年 8 月 5 日  的公告已更新。我们预期该公告中记录的漏洞将在下个版本的 GKE 中修复。
...
bin/cvename.cgi?name=CVE-2019-11247) )，该漏洞允许集群范围的 [ 自定义资源
](https://kubernetes.io/docs/concepts/extend-kubernetes/api-extension/custom-
resources/) 实例得到与存在于所有命名空间内的已命名空间对象一样的处理。这意味着仅有命名空间级 RBAC
权限的用户和服务帐号可以与集群级自定义资源交互。利用此漏洞需要攻击者具有访问任意命名空间内资源的权限。

######  该怎么做？

在包含此漏洞缓解措施的最新补丁程序版本发布后，建议您尽快将集群 [ 升级 ](https://cloud.google.com/kubernetes-
engine/docs/how-to/upgrading-a-cluster?hl=zh_cn) 到该补丁程序版本。我们预期，它们将随下个 GKE
版本在所有地区发布。下面列出了将包含缓解措施的补丁程序版本：
...
1.11 主实例将很快弃用，并计划于 2019 年 7 月 8 日这一周自动升级到 1.12 版本。您可以选择以下任意推荐操作，将节点升级到经过修补的版本。
  ...
  * 在 1.11 节点上启用 [ 自动升级 ](https://cloud.google.com/kubernetes-engine/docs/how-to/node-auto-upgrades?hl=zh_cn) ，允许在主实例升级到 1.12 时升级这些节点。 
...
截至 2019 年 6 月 22 日 21:40（世界协调时间 (UTC)），我们发布了以下经过修补的 Kubernetes 版本。版本介于
...
未经修补的 Linux 内核可能受到远程触发的拒绝服务攻击。 **发送或接收不受信任的网络流量的 Google Kubernetes Engine
节点会受到影响，我们建议您按照下方的缓解步骤保护您的工作负载。**

######  Kubernetes 主实例

  * 使用 [ 授权网络 ](https://cloud.google.com/kubernetes-engine/docs/how-to/authorized-networks?hl=zh_cn) 将流量限制在受信任网络中的 Kubernetes 主实例不受影响。 

  * 由 Google 管理的 GKE 集群主实例将在未来几天内自动得到修补。客户无需采取任何行动。 

######  Kubernetes 节点

将流量限制在受信任网络中的节点不受影响。符合以下条件的集群属于这种情形：

  * 通过防火墙禁止与不受信任的网络通信的节点，或者没有公共 IP 地址的节点（ [ 私有集群 ](https://cloud.google.com/kubernetes-engine/docs/how-to/private-clusters?hl=zh_cn) ） 
  * 没有公共 LoadBalancer 服务的集群 

Google 正在准备这些漏洞的永久性缓解措施，并将作为新节点版本推出。在永久性修复措施可用后，我们将更新本公告，并向所有 GKE 客户发送电子邮件。

在永久性修复措施推出之前，我们创建了一个 Kubernetes DaemonSet，可通过修改主机 ` iptables ` 配置实施缓解措施。
...
**2019 年 7 月 3 日更新** ：此补丁程序在 ` gcloud ` 253.0.0 中提供，适用于 ` kubectl `
...
操作和代码执行访问权限的攻击者修改主机上的文件。此漏洞可能允许攻击者在主机文件系统中替换或创建文件如需了解详情，请参阅 [ Kubernetes 披露信息
](https://groups.google.com/forum/?hl=zh_cn#!topic/kubernetes-security-
...
在即将推出的 ` gcloud ` 版本中将提供经过修补的 ` kubectl ` 版本。您也可以自行 [ 直接升级 ` kubectl `
...
节点均受此漏洞影响，我们建议您在最新补丁程序版本推出后升级到该版本。即将推出的补丁程序版本将包含针对此漏洞的缓解措施。**

**早于版本 1.12.7 的所有 Google Kubernetes Engine (GKE) 主实例都在运行 Docker，会受到此漏洞的影响。** 在
GKE 上，用户不具备主实例上 ` docker cp ` 的访问权限，因此该漏洞给 GKE 主实例造成的风险有限。

#####  该怎么做？

只有在节点运行 Docker 并且发出有可能被劫持的 ` docker cp ` （或 API
对等项）命令时，此类节点才会受到影响。我们预期，Kubernetes 环境中满足这些条件的情况非常少见。运行 [ 带有 containerd 的 COS
...
为经过修补的版本。在补丁程序可用时，您可以启动主实例升级，也可以等待 Google 自动升级主实例。该补丁程序将在 Docker 18.09.7
中提供，并且会包含在即将发布的 GKE 补丁程序中。 **此补丁程序仅在 GKE 1.13 及更高版本中提供。**

我们将按照常规升级节奏，自动将集群主实例升级到经过修补的版本。在经过修补的版本可用后，您也可以自行启动主实例升级。

在包含补丁程序的版本发布后，我们将更新此公告。在 [ 版本说明 ](https://cloud.google.com/kubernetes-
...
在初始公告发布时，仅有 1.13.6-gke.0 到 1.13.6-gke.5 受影响。由于一个回归问题，目前所有 1.13.6.x
版本均受影响。如果您目前在运行 1.13.6 版本，请尽快升级到 1.13.7。
...
1.13.6-gke.0 至 1.13.6-gke.6，我们建议您在不应以 UID 0 的身份运行容器的集群中的所有 Pod 上设置` RunAsUser
` 。 **

如果指定了非根用户 ` USER ` 值（例如，通过在 Dockerfile 中设置 ` USER `
值），则可能发生意外行为。容器在一个节点上首次运行时会正确采用指定 UID。但由于此缺陷的存在，在第二次（及后续）运行的时候，无论指定 UID
...
这些 CVE 统称为微架构数据抽样 (MDS)。这些漏洞可能允许推测性执行与微架构状态交互，从而造成数据泄露风险。如需了解详情，请参阅 [ Intel
...
集群内运行不受信任的代码，否则不会受到影响。

**对于在 Kubernetes Engine 内部自己的多租户服务中运行不受信任代码的用户，这是一个尤为严重的漏洞。** 如需在 Kubernetes
Engine 内缓解此漏洞，请在您的节点内禁用超线程。这些漏洞仅影响使用多个 CPU 的 Google Kubernetes Engine (GKE)
节点。请注意，n1-standard-1（GKE 默认设置）、g1-small 和 f1-micro 虚拟机仅会向客机环境公开 1 个
vCPU，因此无需禁用超线程。

即将发布的 [ 补丁程序版本 ](https://cloud.google.com/kubernetes-engine/release-
notes?hl=zh_cn)
中将包含启用强制刷新功能的附加保护机制。我们将在未来几周内，按常规升级安排将启用自动升级的主实例和节点自动升级到修补后的版本。
...
**警告：**
  ...
  * 可以通过部署一个 DaemonSet 在 GKE 节点池层级停用超线程。但部署此 DaemonSet 将导致您在该节点池中的所有节点同时重新启动。因此，建议在您的集群内创建新节点池，部署 DaemonSet 以在该节点池内停用超线程，随后将工作负载迁移到新节点池。 
...
您必须保持 DaemonSet 在节点池上运行，使得池中创建的新节点可以自动应用更改。节点创建可由节点自动修复、手动或自动升级，以及自动扩缩触发。
...
CVE-2019-11091 ](https://cve.mitre.org/cgi-bin/cvename.cgi?name=2019-11091)
：这些漏洞会利用推测性执行发起攻击。这些 CVE 统称为微架构数据抽样。这些漏洞可能允许推测性执行与微架构状态交互，从而造成数据泄露风险。  |  中  |
...
**在最新补丁程序版本发布后，我们建议您尽快将受影响的集群升级到这些补丁程序版本，同时升级您的 Istio 辅助信息文件（请参见下方说明）。**
...
**鉴于这些漏洞的严重级别，无论您是否启用了节点自动升级，我们都建议您采取以下措施：**
  ...
  2. **按照[ 辅助信息文件升级文档 ](https://istio.io/docs/setup/kubernetes/upgrade/steps/#sidecar-upgrade) 升级您的辅助信息文件。 **

修补后的版本将在今天下午 7:00（美国太平洋夏令时间）之前推出，可供所有 GKE 项目使用。

以下 GKE 版本将提供此补丁程序。在 GKE 安全公告页面上发布相应信息之后（预计在 2019 年 4 月 15
日发布），新集群将默认使用修补后的版本，如果您在此之前创建新集群，则必须为您的集群指定使用修补后的版本。启用了 [ 节点自动升级
](https://cloud.google.com/kubernetes-engine/docs/how-to/node-auto-
upgrades?hl=zh_cn) 的 GKE 客户以及未手动升级的 GKE 客户，其节点会在此后一周内自动升级到修补后的版本。
...
bin/cvename.cgi?name=CVE-2019-9900) 和 [ CVE-2019-9901
](https://cve.mitre.org/cgi-bin/cvename.cgi?name=CVE-2019-9901) 。您可以访问 [ Istio
博客 ](https://istio.io/blog/2019/announcing-1.1.2) 详细了解相关信息。
...
和更新版本中提供。此补丁程序在 1.12 版本中尚不可用。在 [ 版本说明 ](https://cloud.google.com/kubernetes-
engine/docs/release-notes?hl=zh_cn#march_19_2019) 中可以跟踪这些补丁程序的可用情况。

Kubernetes 最近发现了一个新的拒绝服务攻击漏洞 [ CVE-2019-1002100 ](https://cve.mitre.org/cgi-
bin/cvename.cgi?name=CVE-2019-1002100) ，该漏洞允许获得发布补丁程序请求授权的用户编写恶意“json-
patch”请求，过量占用 Kubernetes API 服务器的 CPU 和内存资源，有可能降低集群控制平面的可用性。如需了解详情，请参阅 [
Kubernetes 披露信息 ](https://groups.google.com/forum/?hl=zh_cn#!topic/kubernetes-
announce/vmUUNkYfG9g) 。 **所有 Google Kubernetes Engine (GKE)
主实例都会受到这些漏洞的影响。即将推出的补丁程序版本将包含针对此漏洞的缓解措施。我们将在未来几周内按常规升级安排将集群主实例自动升级到修补后的版本。**

####  该怎么做？

**您无需采取任何操作。GKE 主实例会按照常规升级安排自动升级。** 如果您想尽快升级主实例，可以 [ 手动启动主实例升级
](https://cloud.google.com/kubernetes-engine/docs/how-to/upgrading-a-
cluster?hl=zh_cn#upgrading_the_cluster) 。

我们将更新本公告，列举包含补丁程序的版本。请注意，该补丁程序仅在 1.11 及以上版本中提供，1.10 版本不提供。
...
bin/cvename.cgi?name=CVE-2019-1002100) 漏洞允许用户编写“json-patch”类型的补丁程序，过量占用
Kubernetes API 服务器的 CPU 资源，有可能降低集群控制平面的可用性。
...
若要升级节点，您必须先将主实例升级到最新版本。Kubernetes
1.10.12-gke.7、1.11.6-gke.11、1.11.7-gke.4、1.12.5-gke.5 及更新的版本中提供了该补丁程序。在 [ 版本说明
](https://cloud.google.com/kubernetes-engine/docs/release-
notes?hl=zh_cn#february-11-2019) 中可以跟踪这些补丁程序的可用情况。
...
**2019 年 2 月 25 日更新** ：正如之前所宣布的，1.11.7-gke.4 不提供该补丁程序。如果您运行的是 1.11.7，您可以选择：降级到
1.11.6、升级到 1.12，或者等到 2019 年 3 月 4 日这一周发布的下一个 1.11.7 补丁程序。
...
(DoS) 漏洞。在 Google Kubernetes Engine (GKE) 中，该漏洞可能允许用户编写恶意请求，过量占用 Kubernetes
API 服务器的 CPU 资源，有可能降低集群控制平面的可用性。如需了解详情，请参阅 [ Go 编程语言披露信息
](https://groups.google.com/forum/?hl=zh_cn#!topic/golang-
announce/mVeX35iXuSw) 。

**所有 Google Kubernetes Engine (GKE) 主实例都会受到这些漏洞的影响。[ 最新版本的补丁程序
](https://cloud.google.com/kubernetes-engine/docs/release-
notes?hl=zh_cn#february-11-2019)
包含针对此漏洞的缓解措施。我们将在未来几周内，按常规升级安排将集群主实例自动升级到修补后的版本。 **
...
CVE-2019-6486 是 P-521 和 P-384 椭圆曲线的加密/椭圆实现中存在的一个漏洞。用户可以利用该漏洞编写输入来过量占用 CPU 资源。
...
engine/docs/release-notes?hl=zh_cn#november-12-2018) 。您无需采取任何操作。 **
...
中的评级为“严重”。考虑到 GKE 实现中的一些细节会阻止未通过身份验证的提权路径，此漏洞在 GKE 中的评级为“高”。
...
这些 L1TF 漏洞通过攻击处理器层的数据结构配置来利用推测执行。“L1”是指 1 级数据缓存 (L1D)，这是一种用于加快内存访问速度的小型芯片上资源。
...
一样，这也是一个内核级的网络漏洞，可提高拒绝服务攻击 (DoS) 对较薄弱的系统的破坏力。两者的主要区别是 CVE-2018-5391 可通过 IP
...
截至 2018 年 8 月 11 日，所有 Kubernetes Engine 主实例都可以抵御对这两个漏洞的攻击。此外，所有配置为自动升级的
...
如果满足以下所有条件，则此漏洞会影响到您：
  ...
  * 由不受信任的用户创建的 Pod 受到限制，不能访问主机根目录（例如，通过 [ PodSecurityPolicy ](https://cloud.google.com/kubernetes-engine/docs/how-to/pod-security-policies?hl=zh_cn) ）。 
  * 允许不受信任的用户创建的 Pod 使用 gitRepo 卷类型。 
...
补丁程序将包含在即将发布的 Kubernetes Engine 版本中。请稍后查看此页面了解详情。
...
最近在 Linux 内核中发现了多个漏洞，这些漏洞可能会允许无特权的进程提升其权限或（通过内核崩溃）实施拒绝服务攻击。这些 CVE 标识为 [
...
若要升级，您必须先将主实例升级到最新版本。Kubernetes Engine 1.8.12-gke.1、Kubernetes Engine
...
内核。它允许无特权的用户或进程引发系统内核崩溃，继而导致 DoS 攻击或权限提升。此漏洞被评为高危漏洞，CVSS 评分为 7.8。

[ CVE-2018-8897 ](https://cve.mitre.org/cgi-
bin/cvename.cgi?name=CVE-2018-8897) ：此漏洞影响到 Linux 内核。它允许无特权的用户或进程引发系统内核崩溃，继而导致
DoS 攻击。此漏洞被评为中危漏洞，CVSS 评分为 6.5。
...
Engine 不会受到影响。此漏洞被评为高危漏洞，CVSS 评分为 8.0。
...
，这些漏洞允许容器访问其以外的文件。所有 Kubernetes Engine
...
若要升级，您必须先将主实例升级到最新版本。Kubernetes 1.9.4-gke.1、Kubernetes 1.8.9-gke.1 和
...
卷装载功能的容器访问卷外部的文件。这意味着，如果您使用 PodSecurityPolicy 来阻止容器访问主机路径卷，能够更新或创建 Pod
...
如需详细了解此修复程序，请参阅该 [ Kubernetes 博文
