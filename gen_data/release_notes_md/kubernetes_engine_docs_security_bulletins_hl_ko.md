...
Google Kubernetes Engine(GKE)에 해당하는 모든 보안 게시판은 이 주제에서 확인할 수 있습니다.
...
**참고:** GKE에서 멀티테넌트 워크로드를 실행할 경우 해당 게시판에 각별히 유의하시기 바랍니다. 취약점이 멀티테넌트 워크로드에 영향을
미칠 가능성이 크기 때문입니다. GKE의 보안 경계에 대한 기술적 설명과 이에 따라 워크로드가 받는 영향에 대해서는 [ 다양한
Kubernetes 스택 레이어에서의 격리
...
**대부분의 고객은 별도의 조치를 취하지 않아도 됩니다. Windows Server를 실행 중인 노드만 영향을 받습니다.**

Windows Server 노드를 사용하고 있는 고객의 경우 노드와 해당 노드에서 실행되는 컨테이너화된 워크로드를 모두 패치 버전으로
업데이트해야 이 취약점을 해결할 수 있습니다.
...
패치 버전을 출시하기 위한 과정이 이미 진행 중이며 2020년 1월 24일까지 완료될 예정입니다.
...
공격자가 중간자 공격을 수행하고 영향을 받는 소프트웨어에 대한 TLS 연결에서 기밀 정보를 복호화하는 데 악용될 수 있습니다.
...
**Kubernetes Engine을 실행하는 호스트 인프라는 고객의 워크로드를 격리합니다. 자체 멀티테넌트 GKE 클러스터 내에서 신뢰할
수 없는 코드를 실행 중인 경우 _및_ N2, M2, C2 노드를 사용하는 경우가 아니라면 추가 조치는 필요하지 않습니다. ** N1 노드의
GKE 인스턴스에도 새로운 조치가 필요하지 않습니다.

온프렘으로 배포된 Anthos GKE를 실행 중인 경우 노출 여부는 하드웨어에 따라 다릅니다. [ Intel 정보 공개
](https://blogs.intel.com/technology/2019/11/ipas-november-2019-intel-
platform-update-ipu/) 와 인프라를 비교해 보세요.

####  어떻게 해야 하나요?

**N2, M2, C2 노드가 포함된 노드 풀을 사용 중 _이고_ 이러한 노드가 자체 멀티테넌트 GKE 클러스터 내에서 신뢰할 수 없는
...
: 이 CVE는 TSX Async Abort(TAA)로도 알려져 있습니다. TAA는 [ 마이크로아키텍처 데이터 샘플링(MDS)
](https://cloud.google.com/kubernetes-engine/docs/security-
bulletins?hl=ko#may-14-2019) 에서 악용한 것과 동일한 마이크로아키텍처 데이터 구조로 데이터를 유출할 수 있는 또 다른
경로를 제공합니다.
...
bin/cvename.cgi?name=CVE-2019-16276) 에서 설명된 바와 같이 Go 프로그래밍 언어에서 최근에 취약점이
발견되었습니다. 이 취약점은 Authenticating Proxy를 사용하는 Kubernetes 구성에 영향을 줄 수 있습니다. 자세한
내용은 [ Kubernetes 정보 공개
...
수정이 포함된 패치 버전이 제공되는 즉시 클러스터를 업데이트하는 것이 좋습니다. 패치 버전은 10월 14일이 포함된 주에 계획된 GKE
...
Go 프로그래밍 언어에서 신규 보안 취약점 [ CVE-2019-9512 ](https://cve.mitre.org/cgi-
bin/cvename.cgi?name=CVE-2019-9512) 와 [ CVE-2019-9514
](https://cve.mitre.org/cgi-bin/cvename.cgi?name=CVE-2019-9514) 를 최근 발견했으며, 둘
모두 서비스 거부(DoS) 취약점입니다. GKE에서 사용자는 이 취약점을 이용해 Kubernetes API 서버의 CPU를 과도하게 소비하는
악의적인 요청을 만들어서 클러스터 제어 영역의 가용성을 줄일 수 있습니다. 자세한 내용은 [ Go 프로그래밍 언어 정보 공개
...
2019년 5월 31일  게시판에 설명된 취약점을 해결하는 수정에 관한 게시판이 업데이트되었습니다.

##  2019년 8월 22일

2019년 8월 5일  게시판이 업데이트되었습니다. 이전 게시판에 설명된 취약점을 해결하는 수정이 [ 제공
](https://cloud.google.com/kubernetes-engine/docs/release-
notes?hl=ko#august_22_2019) 됩니다.

##  2019년 8월 8일

2019년 8월 5일  게시판이 업데이트되었습니다. 해당 게시판에 설명된 취약점을 해결하는 수정은 다음 GKE 출시에서 제공될 예정입니다.
...
Kubernetes에서 [ CVE-2019-11247 ](https://cve.mitre.org/cgi-
bin/cvename.cgi?name=CVE-2019-11247) 취약점을 최근 발견했으며, 이 취약점은 클러스터 범위의 [ 커스텀 리소스
](https://kubernetes.io/docs/concepts/extend-kubernetes/api-extension/custom-
resources/) 인스턴스가 모든 네임스페이스에 존재하는 네임스페이스화된 객체인 것처럼 처리될 수 있다는 문제를 안고 있습니다. 즉,
사용자 및 서비스 계정은 네임스페이스 수준의 RBAC 권한만 있어도 클러스터 범위의 커스텀 리소스와 상호작용할 수 있습니다. 공격자에게
...
**참고:** ` kubectl ` 1.11.10에서는 이 패치를 사용할 수 없습니다.
...
1.11 마스터는 곧 지원 중단되며 2019년 7월 8일이 포함된 주에 1.12로 자동 업그레이드될 예정입니다. 다음 권장 조치 중 하나를
...
Google은 이러한 취약점을 영구적으로 완화할 방법을 준비 중이며 새 노드 버전으로 제공할 예정입니다. 영구 수정이 제공되면 이 게시판을
업데이트하고 모든 GKE 고객에게 이메일을 보내 드릴 것입니다.

영구 수정이 제공되기 전에 사용할 수 있도록 호스트 ` iptables ` 구성을 수정하여 완화 방법을 구현하는 Kubernetes
DaemonSet가 제공됩니다.
...
Kubernetes에서 컨테이너 내 ` kubectl cp ` 작업과 코드 실행에 대한 액세스 권한이 있는 공격자가 호스트의 파일을 수정할
수 있는 [ CVE-2019-11246 ](https://cve.mitre.org/cgi-
bin/cvename.cgi?name=CVE-2019-11246) 취약점을 최근 발견했습니다. 공격자는 이 취약점을 악용하여 호스트 파일
시스템에서 파일을 바꾸거나 만들 수 있습니다. 자세한 내용은 [ Kubernetes 정보 공개
](https://groups.google.com/forum/?hl=ko#!topic/kubernetes-security-
...
](https://cve.mitre.org/cgi-bin/cvename.cgi?name=CVE-2019-11246) 을 이용하여 호스트의
...
Docker에서 [ CVE-2018-15664 ](https://cve.mitre.org/cgi-
bin/cvename.cgi?name=CVE-2018-15664) 취약점을 최근 발견했으며, 이 취약점은 컨테이너 내부에서 코드를 실행할 수
있는 공격자가 외부에서 시작된 ` docker cp ` 작업을 도용할 수 있다는 문제를 안고 있습니다. 공격자는 이 취약점을 악용하여 파일이
작성된 위치를 호스트 파일 시스템에 있는 임의 위치로 변경할 수 있습니다.

**Docker를 실행하는 모든 Google Kubernetes Engine(GKE) 노드가 이 취약점의 영향을 받으므로 최신 패치 버전이
제공되는 대로 업그레이드하는 것이 좋습니다. 예정된 패치 버전에 이 취약점의 완화 방법이 포함됩니다.**
...
Kubernetes 프로젝트에서 kubelet v1.13.6 및 v1.14.2의 취약점 [ CVE-2019-11245
](https://cve.mitre.org/cgi-bin/cvename.cgi?name=2019-11245) 를 공개했으며, 이로 인해
컨테이너 이미지에 다른 사용자가 지정되었더라도 컨테이너가 UID 0(일반적으로 ` root ` 사용자에 매핑)으로 실행될 수 있습니다.
**컨테이너가 루트가 아닌 사용자로 실행되고 노드 버전 1.13.6-gke.0~1.13.6-gke.6을 실행하고 있다면 컨테이너가 UID
0으로 실행되어서는 안 되는 클러스터의 모든 Pod에` RunAsUser ` 를 설정하는 것이 좋습니다. **
...
출력에 아래와 같은 kubelet 버전이 나열되면 영향을 받는 노드인 것입니다.
...
**Kubernetes Engine을 실행하는 호스트 인프라는 고객의 워크로드를 서로 격리합니다. 자체 멀티테넌트 GKE 클러스터에서 신뢰할
수 없는 코드를 실행하는 경우가 아니라면 영향을 받지 않습니다.**

**Kubernetes Engine 내 자체 멀티테넌트 서비스에서 신뢰할 수 없는 코드를 실행하는 고객에게는 특히 심각한 취약점입니다.**
...
**자체 멀티테넌트 GKE 클러스터에서 신뢰할 수 없는 코드를 실행하는 경우가 아니라면 영향을 받지 않습니다.**
...
cluster?hl=ko#upgrading_the_cluster) 해야 합니다. GKE 마스터는 정기적인 업그레이드 주기에 따라 자동
업그레이드됩니다.
...
클러스터에서 기본적으로 이 패치 버전을 사용합니다. 그 전에 새 클러스터를 만들었다면 사용할 패치 버전을 지정해야 합니다. [ 노드 자동
업그레이드 ](https://cloud.google.com/kubernetes-engine/docs/how-to/node-auto-
upgrades?hl=ko) 를 사용 설정했으며 수동으로 업그레이드 하지 않는 GKE 고객의 노드는 그 다음 주에 패치 버전으로 자동
업그레이드됩니다.
...
Kubernetes에서 신규 서비스 거부 취약점 [ CVE-2019-1002100 ](https://cve.mitre.org/cgi-
bin/cvename.cgi?name=CVE-2019-1002100) 을 최근 발견했으며, 이 취약점은 패치 요청 권한이 있는 사용자가
Kubernetes API 서버의 CPU와 메모리를 과도하게 소비하는 악의적인 'json-patch' 요청을 만들어 클러스터 제어 영역의
가용성을 줄일 수 있다는 문제를 안고 있습니다. 자세한 내용은 [ Kubernetes 정보 공개
...
이 게시판은 업데이트되어 패치가 포함된 버전이 추가될 예정입니다. 이 패치는 버전 1.10이 아닌 버전 1.11+에서만 사용할 수 있습니다.
...
notes?hl=ko#february-11-2019) 에서 이 패치의 가용성을 추적할 수 있습니다.
...
실행하고 있다면 1.11.6으로 다운그레이드 또는 1.12로 업그레이드하거나 2019년 3월 4일이 포함된 주에 다음 1.11.7 패치가
제공될 때까지 기다릴 수 있습니다.

Go 프로그래밍 언어에서 신규 보안 취약점 [ CVE-2019-6486 ](https://cve.mitre.org/cgi-
bin/cvename.cgi?name=CVE-2019-6486) 을 최근 발견했으며, 이는 P-521 및 P-384 타원 곡선의 암호화/타원
구현에 관한 서비스 거부(DoS) 취약점입니다. Google Kubernetes Engine(GKE)에서 사용자는 이 취약점을 악용해
Kubernetes API 서버의 CPU를 과도하게 소비하는 악의적인 요청을 만들어서 클러스터 제어 영역의 가용성을 줄일 수 있습니다.
자세한 내용은 [ Go 프로그래밍 언어 정보 공개
](https://groups.google.com/forum/?hl=ko#!topic/golang-announce/mVeX35iXuSw) 를
참조하세요.

**이 취약점은 모든 Google Kubernetes Engine(GKE) 마스터에 영향을 미칩니다.[ 최신 패치 버전
](https://cloud.google.com/kubernetes-engine/docs/release-
notes?hl=ko#february-11-2019) 에 이 취약점의 완화 방법이 포함되어 있습니다. 몇 주 후 정기적인 업그레이드 주기에
따라 클러스터 마스터가 패치 버전으로 자동 업그레이드됩니다. **
...
Kubernetes에서 신규 보안 취약점 [ CVE-2018-1002105 ](https://cve.mitre.org/cgi-
bin/cvename.cgi?name=CVE-2018-1002105) 를 최근 발견했으며, 이 취약점은 권한이 비교적 낮은 사용자가
kubelet의 API에 대한 승인을 우회하여 클러스터 내의 어떤 노드에서든 모든 Pod에 대해 임의의 작업을 실행할 수 있다는 문제를 안고
있습니다. 자세한 내용은 [ Kubernetes 정보 공개
](https://groups.google.com/forum/?hl=ko#!topic/kubernetes-
...
영향을 받았을 가능성이 있는 모든 토큰은 다음 주 중에 단계적으로 취소될 예정입니다. 취소가 완료되면 이 게시판이 업데이트됩니다.
**사용자의 추가 조치는 필요하지 않습니다.** (이 순환은 2018년 11월 16일에 완료됨)
...
bin/cvename.cgi?name=CVE-2018-5391) 이 최근 공개되었습니다. 이는 [ CVE-2018-5390
...
2018년 8월 11일 이전인 Kubernetes Engine 노드 풀은 두 취약점의 영향을 받을 수 있습니다.

####  패치 버전

이 취약점의 심각도가 높으므로 패치가 제공되는 즉시 노드를 [ 수동으로 업그레이드
](https://cloud.google.com/kubernetes-engine/docs/how-to/upgrading-a-
cluster?hl=ko#upgrading_nodes) 하는 것이 좋습니다.
...
Git에서 권한 없는 사용자가 gitRepo 볼륨이 포함된 Pod를 생성할 수 있을 경우 Kubernetes 내의 권한 에스컬레이션이 가능할
수 있는 취약점이 최근 발견되었습니다. 이 CVE는 [ CVE-2018-11235 ](https://cve.mitre.org/cgi-
bin/cvename.cgi?name=CVE-2018-11235) 태그로 식별됩니다.
...
Linux 커널에서 권한 없는 프로세스의 (커널 장애를 통한) 서비스 거부 또는 권한 에스컬레이션이 가능할 수 있는 몇 가지 취약점이 최근
발견되었습니다. 이러한 CVE는 [ CVE-2018-1000199 ](https://cve.mitre.org/cgi-
...
버전에는 Container-Optimized OS와 Ubuntu 이미지의 패치가 포함되어 있습니다.
...
CVSS 8.0점에 해당하는 높은 등급의 취약점으로 분류됩니다.
...
Kubernetes 프로젝트에서 컨테이너가 컨테이너 외부에 있는 파일에 액세스할 수 있는 신규 보안 취약점 [ CVE-2017-1002101
](https://cve.mitre.org/cgi-bin/cvename.cgi?name=2017-1002101) 과 [
CVE-2017-1002102 ](https://cve.mitre.org/cgi-
bin/cvename.cgi?name=2017-1002102) 를 [ 최근 공개
](https://groups.google.com/forum/?hl=ko#!topic/kubernetes-security-
announce/P7lBjbjDKd8) 했습니다. 이 취약점은 모든 Kubernetes Engine 노드에 영향을 미치므로 아래의 세부정보를
참고하여 최대한 빨리 최신 패치 버전으로 업그레이드하는 것이 좋습니다.
...
hostpath 볼륨 액세스를 차단하는 경우 Pod 업데이트나 생성이 가능한 공격자가 다른 볼륨 유형으로 임의의 hostpath를 마운트할
수 있게 됩니다.
...
수정에 대한 자세한 내용은 [ Kubernetes 블로그 게시물
