...
See also:
  ...
  * [ Upgrading GKE on-prem ](/anthos/gke/docs/on-prem/archive/1.2/how-to/upgrading)
...
to/admin-workstation#copy_terraform) . vSphere provider version 1.16
...
prem/reference/config) to say that the minimum number of worker nodes is
three.
...
prem/how-to/preflight-checks#known_issue/anthos/gke/docs/on-prem/how-
to/preflight-checks#list) that validates an NTP server is available.
...
to/preflight-checks#known_issue) from 1.2.0-gke.6 that prevented ` gkectl
check-config ` from performing all of its validations against clusters in
nested resource pools or the default resource pool.
...
prem/docs/release-notes#september_26_2019) , the ` gkeconnect.proxy ` field is
no longer in the GKE on-prem [ configuration file
](https://cloud.google.com/gke-on-prem/docs/reference/config) .
...
checks) command can fail with this error:
...
GKE on-prem ](https://cloud.google.com/gke-on-prem/docs/how-to/upgrading) .
...
prem/docs/concepts/networking#support_for_vmware_nsx-t) .
...
to/preflight-checks) for validating your GKE on-prem configuration file before
your create your clusters. These new checks can validate that your Google
Cloud project, vSphere network, and other elements of your environment are
correctly configured.

**FEATURE:**

Published [ basic installation ](https://cloud.google.com/gke-on-
prem/docs/how-to/install-overview-basic) workflow. This workflow offers a
simplified workflow for quickly installing GKE on-prem using static IPs.

**FEATURE:**

Published guidelines for [ installing Container Storage Interface
](https://cloud.google.com/gke-on-prem/docs/how-to/install-csi-driver) (CSI)
drivers. CSI enables using storage devices not natively supported by
Kubernetes.

**FEATURE:**

Updated documentation for [ authenticating using OpenID Connect (OIDC)
](https://cloud.google.com/gke-on-prem/docs/how-to/oidc) with the Anthos
...
](https://cloud.google.com/gke-on-prem/docs/downloads#latest) . Then, see [
Upgrading admin workstation ](https://cloud.google.com/gke-on-prem/docs/how-
to/upgrading) and [ Upgrading clusters ](https://cloud.google.com/gke-on-
prem/docs/how-to/upgrading) .
...
prem/docs/how-to/security/hardening-your-cluster) .
...
admin-workstation) . Then, you should [ upgrade your clusters
](https://cloud.google.com/gke-on-prem/docs/how-to/administration/upgrading-
clusters) .
...
](https://cloud.google.com/gke-on-prem/docs/support/toolbox) .
...
Upgrading clusters ](https://cloud.google.com/gke-on-prem/docs/how-
to/upgrading) .
...
feature—perform the following steps _before_ you upgrade your user clusters:
  ...
  2. Under the ` usercluster ` specification, add the ` antiaffinitygroups ` field as described in the [ ` antiaffinitygroups ` documentation ](https://cloud.google.com/gke-on-prem/docs/how-to/installation/install#antiaffinitygroups) : ` usercluster: ... antiaffinitygroups: enabled: false `
...
prem/docs/how-to/administration/default-storage-class) for your clusters.
...
user-cluster#delete_unhealthy_cluster) with ` gkectl delete cluster --force `

**FEATURE:**

You can now [ diagnose node issues ](https://cloud.google.com/gke-on-
prem/docs/support/debug-toolbox) using the ` debug-toolbox ` container image.

**FEATURE:**

You can now [ skip validatations ](https://cloud.google.com/gke-on-
prem/docs/how-to/installation/install#skip_validate) run by ` gkectl `
commands.
...
prem/docs/how-to/installation/install#antiaffinitygroups) .
...
prem/docs/support/diagnose#diagnose_snapshot) .
...
prem/docs/how-to/installation/install) .
...
to/administration/deleting-a-user-cluster) .
...
prem/docs/security-bulletins#august-22-2019) .

**FIXED:**

[ Fixed a vulnerability in RBAC proxy ](https://cloud.google.com/gke-on-
prem/docs/security-bulletins#august-23-2019) .
...
prem/docs/reference/cheatsheet) .
...
to/upgrading/from-beta) , and review the following points:
...
](https://cloud.google.com/gke-on-prem/docs/how-to/installation/manual-lb) to
configure a L4 load balancer. You can still choose to use the default load
balancer, F5 BIG-IP.
...
](https://cloud.google.com/gke-on-prem/docs/overview#config) .
...
to/installation/install#generate_config) .
...
prem/docs/how-to/installation/install#prepare) .
...
clusters) can cause disruption or downtime for workloads that use [
...
prem/docs/beta-1.4/how-to/administration/diagnose#capture_admin) command, the
` --admin-ssh-key-path ` parameter is now optional.
...
prem/docs/beta-1.4/how-to/administration/upgrading-a-cluster) :
...
to/administration/upgrading-a-cluster#upgrading_the_ingress_controller) . The
ingress controller runs in the ` gke-system ` namespace for both admin and
user clusters. This enables easier TLS management for Ingress. To enable
...
prem/docs/beta-1.3/how-to/installation/install#enabling_ingress) .
...
](https://cloud.google.com/kubernetes-engine/connect/) .
...
to/installation/getting-started#download_ova) , a pre-configured virtual
...
adfs) and [ Authentication ](https://cloud.google.com/gke-on-
prem/docs/concepts/authentication) .
...
[ Connect documentation ](https://cloud.google.com/kubernetes-engine/connect/)
has been migrated.
...
prem/docs/beta-1.0/how-to/administration/upgrading-a-cluster) to future beta
versions.

**FEATURE:**

[ Anthos Config Management ](https://cloud.google.com/anthos-config-
management/docs/) version 0.11.6 is now available.
...
requirements ](https://cloud.google.com/gke-on-prem/docs/requirements) .
