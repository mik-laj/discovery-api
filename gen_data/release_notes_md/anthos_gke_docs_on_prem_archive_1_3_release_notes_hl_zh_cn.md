您正在查看旧版 Anthos GKE On-Prem 的文档。 [ 查看最新文档
](https://cloud.google.com/anthos/gke/docs/on-prem?hl=zh-cn) 。

#  版本说明

本页面记录了 Anthos GKE On-Prem 的正式版更新。您可以定期查看此页面，了解有关新增功能、功能更新、问题修复、已知问题和弃用功能的公告。
...
gpus?hl=zh-cn) , users can now manually attach a GPU to a worker node VM to
...
to/upgrading?hl=zh-cn) . GKE on-prem 1.3.3-gke.0 clusters run on Kubernetes
1.15.12-gke.9.
...
**Fixes:**
  ...
  * Fixed [ CVE-2020-8559 ](https://cve.mitre.org/cgi-bin/cvename.cgi?name=CVE-2020-8559) described in [ Security bulletins ](https://cloud.google.com/anthos/gke/docs/on-prem/security-bulletins?hl=zh-cn#gcp-2020-009) . 
...
GKE on-prem ](https://cloud.google.com/anthos/gke/docs/on-prem/how-
to/upgrading?hl=zh-cn) . GKE on-prem 1.4.0-gke.13 clusters run on Kubernetes
1.16.8-gke.6.
...
**Updated to Kubernetes 1.16:**
  ...
  * Please note that Kubernetes 1.16 has deprecated some of its APIs. For more information, see [ Kubernetes 1.16 deprecated APIs ](https://cloud.google.com/kubernetes-engine/docs/deprecations/apis-1-16?hl=zh-cn) . 
...
**Simplified upgrade:**
  ...
  * This release provides a simplified upgrade experience via the following changes: 
    ...
    * Support skip version upgrade to enable users to upgrade the cluster from any patch release of a minor release to any patch release of the next minor release. For more information about the detailed upgrade procedure and limitations, see [ upgrading GKE on-prem ](https://cloud.google.com/anthos/gke/docs/on-prem/how-to/upgrading?hl=zh-cn) . 
    * The [ alternate upgrade scenario for Common Vulnerabilities and Exposures ](https://cloud.google.com/anthos/gke/docs/on-prem/how-to/upgrading?hl=zh-cn) has been deprecated. All upgrades starting with version 1.3.2 need to upgrade the entire admin workstation. 
...
**Improved installation and cluster configuration:**
  ...
  * The user cluster [ node pools ](https://cloud.google.com/anthos/gke/docs/on-prem/how-to/user-cluster-basic?hl=zh-cn#nodepoolsname) feature is now generally available. 
...
**Improved disaster recovery capabilities:**
  ...
  * This release provides enhanced disaster recovery functionality to [ support backup and restore ](https://cloud.google.com/anthos/gke/docs/on-prem/how-to/backing-up?hl=zh-cn#user_cluster_backups) HA user cluster with etcd. 
...
**Enhanced monitoring with Cloud Monitoring (formerly Stackdriver):**
  ...
  * This release provides better product monitoring and resource usage management via the following changes: 
    ...
    * Introduces a [ default monitoring dashboard ](https://cloud.google.com/anthos/gke/docs/on-prem/how-to/logging-and-monitoring?hl=zh-cn#dashboards) . 
...
**Improved scale:**
  ...
  * This release improves the cluster scalability by supporting [ a maximum of 10 instead of 5 user clusters for each admin cluster ](https://cloud.google.com/anthos/gke/docs/on-prem/quotas?hl=zh-cn) . 
...
checks?hl=zh-cn) with any GKE on-prem [ download image
](https://cloud.google.com/anthos/gke/docs/on-prem/downloads?hl=zh-cn#bundle-
latest) .
...
prem/how-to/upgrading?hl=zh-cn#alternate_upgrade_scenario) , a timeout might
...
](https://cloud.google.com/anthos/gke/docs/on-prem/how-to/oidc?hl=zh-cn) has
...
](https://cloud.google.com/anthos/gke/docs/on-prem/quotas?hl=zh-cn) .
...
ips?hl=zh-cn#bundlepath) | [ DHCP
](http://cloud.google.com/anthos/gke/docs/on-prem/how-to/install-dhcp?hl=zh-
cn#bundlepath) ), you must append the ` --fast ` flag to run ` gkectl check-
...
prem/reference/gkectl/diagnose?hl=zh-cn) command might fail if your clusters:
...
to/admin-workstation?hl=zh-cn#copy_terraform) . vSphere provider version 1.16
...
prem/reference/config?hl=zh-cn) to say that the minimum number of worker nodes
is three.
...
prem/how-to/preflight-checks?hl=zh-cn#list) that validates an NTP server is
available.
...
to/preflight-checks?hl=zh-cn#known_issue) from 1.2.0-gke.6 that prevented `
...
prem/release-notes?hl=zh-cn#January_28_2020) from 1.2.0-gke.6 that affects
...
prem/docs/release-notes?hl=zh-cn#september_26_2019) , the ` gkeconnect.proxy `
field is no longer in the GKE on-prem [ configuration file
](https://cloud.google.com/gke-on-prem/docs/reference/config?hl=zh-cn) .
...
checks?hl=zh-cn) command can fail with this error:
...
](https://cloud.google.com/gke-on-prem/docs/concepts/networking?hl=zh-
cn#support_for_vmware_nsx-t) .
...
to/preflight-checks?hl=zh-cn) for validating your GKE on-prem configuration
...
prem/docs/how-to/install-overview-basic?hl=zh-cn) workflow. This workflow
...
](https://cloud.google.com/gke-on-prem/docs/how-to/install-csi-driver?hl=zh-
cn) (CSI) drivers. CSI enables using storage devices not natively supported by
Kubernetes.

**FEATURE:**

Updated documentation for [ authenticating using OpenID Connect (OIDC)
](https://cloud.google.com/gke-on-prem/docs/how-to/oidc?hl=zh-cn) with the
...
](https://cloud.google.com/gke-on-prem/docs/downloads?hl=zh-cn#latest) . Then,
see [ Upgrading admin workstation ](https://cloud.google.com/gke-on-
prem/docs/how-to/upgrading?hl=zh-cn) and [ Upgrading clusters
](https://cloud.google.com/gke-on-prem/docs/how-to/upgrading?hl=zh-cn) .
...
prem/docs/how-to/security/hardening-your-cluster?hl=zh-cn) .
...
clusters?hl=zh-cn#admin_datadisk_folder) and installation topics.
...
admin-workstation?hl=zh-cn) . Then, you should [ upgrade your clusters
](https://cloud.google.com/gke-on-prem/docs/how-to/administration/upgrading-
clusters?hl=zh-cn) .
...
](https://cloud.google.com/gke-on-prem/docs/support/toolbox?hl=zh-cn) .
...
](https://cloud.google.com/gke-on-prem/docs/security-bulletins?hl=zh-
cn#october-16-2019) .
...
feature—perform the following steps _before_ you upgrade your user clusters:
  ...
  2. Under the ` usercluster ` specification, add the ` antiaffinitygroups ` field as described in the [ ` antiaffinitygroups ` documentation ](https://cloud.google.com/gke-on-prem/docs/how-to/installation/install?hl=zh-cn#antiaffinitygroups) : ` usercluster: ... antiaffinitygroups: enabled: false `
...
prem/docs/how-to/administration/default-storage-class?hl=zh-cn) for your
clusters.
...
user-cluster?hl=zh-cn#delete_unhealthy_cluster) with ` gkectl delete cluster
--force `

**FEATURE:**

You can now [ diagnose node issues ](https://cloud.google.com/gke-on-
prem/docs/support/debug-toolbox?hl=zh-cn) using the ` debug-toolbox `
container image.

**FEATURE:**

You can now [ skip validatations ](https://cloud.google.com/gke-on-
prem/docs/how-to/installation/install?hl=zh-cn#skip_validate) run by ` gkectl
` commands.
...
prem/docs/how-to/installation/install?hl=zh-cn#antiaffinitygroups) .
...
prem/docs/support/diagnose?hl=zh-cn#diagnose_snapshot) .
...
prem/docs/how-to/installation/install?hl=zh-cn) .
...
prem/docs/security-bulletins?hl=zh-cn#august-22-2019) .

**FIXED:**

[ Fixed a vulnerability in RBAC proxy ](https://cloud.google.com/gke-on-
prem/docs/security-bulletins?hl=zh-cn#august-23-2019) .
...
prem/docs/reference/cheatsheet?hl=zh-cn) .
...
to/upgrading/from-beta?hl=zh-cn) , and review the following points:
...
lb?hl=zh-cn) to configure a L4 load balancer. You can still choose to use the
...
](https://cloud.google.com/gke-on-prem/docs/overview?hl=zh-cn#config) .
...
](https://cloud.google.com/gke-on-prem/docs/how-to/installation/install?hl=zh-
cn#generate_config) .
...
prem/docs/how-to/installation/install?hl=zh-cn#prepare) .
...
to/administration/upgrading-clusters?hl=zh-cn) to this version. You can no
...
clusters?hl=zh-cn) can cause disruption or downtime for workloads that use [
...
to/installation/manual-lb?hl=zh-cn) to GKE On-Prem version 1.0.10. To upgrade
...
prem/docs/beta-1.4/how-to/administration/diagnose?hl=zh-cn#capture_admin)
...
engine/connect/updating-agent?hl=zh-cn) :
...
prem/docs/beta-1.4/how-to/administration/upgrading-a-cluster?hl=zh-cn) :
...
to/administration/diagnose?hl=zh-cn#performing_a_dry_run_for_a_snapshot) flag.
...
to/administration/upgrading-a-cluster?hl=zh-
cn#upgrading_the_ingress_controller) . The ingress controller runs in the `
...
to/installation/install?hl=zh-cn#enabling_ingress) .
...
](https://cloud.google.com/kubernetes-engine/connect/?hl=zh-cn) .
...
to/installation/getting-started?hl=zh-cn#download_ova) , a pre-configured
...
to/administration/backing-up?hl=zh-cn) .
...
adfs?hl=zh-cn) and [ Authentication ](https://cloud.google.com/gke-on-
prem/docs/concepts/authentication?hl=zh-cn) .
...
engine/connect/?hl=zh-cn) has been migrated.
...
prem/docs/beta-1.0/how-to/administration/upgrading-a-cluster?hl=zh-cn) to
future beta versions.

**FEATURE:**

[ Anthos Config Management ](https://cloud.google.com/anthos-config-
management/docs/?hl=zh-cn) version 0.11.6 is now available.
...
to/installation/getting-started?hl=zh-cn) has been updated with steps for
...
requirements ](https://cloud.google.com/gke-on-prem/docs/requirements?hl=zh-
cn) .
