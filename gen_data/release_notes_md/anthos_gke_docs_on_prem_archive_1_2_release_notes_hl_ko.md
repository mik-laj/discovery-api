...
참고 항목
  ...
  * [ GKE On-Prem 업그레이드 ](https://cloud.google.com/anthos/gke/docs/on-prem/archive/1.2/how-to/upgrading?hl=ko)
...
추가하거나 피드 URL을 다음과 같이 직접 추가하세요. ` https://cloud.google.com/feeds/gkeonprem-
release-notes.xml `
...
gpus?hl=ko) , users can now manually attach a GPU to a worker node VM to run
...
to/upgrading?hl=ko) . GKE on-prem 1.3.3-gke.0 clusters run on Kubernetes
1.15.12-gke.9.
...
**Fixes:**
  ...
  * Fixed [ CVE-2020-8559 ](https://cve.mitre.org/cgi-bin/cvename.cgi?name=CVE-2020-8559) described in [ Security bulletins ](https://cloud.google.com/anthos/gke/docs/on-prem/security-bulletins?hl=ko#gcp-2020-009) . 
...
GKE on-prem ](https://cloud.google.com/anthos/gke/docs/on-prem/how-
to/upgrading?hl=ko) . GKE on-prem 1.4.1-gke.1 clusters run on Kubernetes
1.16.9-gke.14.
...
**Fixes:**
  ...
  * Updated the default value for ` ingressHTTPNodePort ` and ` ingressHTTPSNodePort ` in the ` loadBalancer.manualLB ` section of the [ admin cluster configuration ](https://cloud.google.com/anthos/gke/docs/on-prem/how-to/admin-cluster-configuration-file?hl=ko) file. 
  * Fixed [ CVE-2020-8558 ](https://cve.mitre.org/cgi-bin/cvename.cgi?name=CVE-2020-8558) and [ CVE-2020-8559 ](https://cve.mitre.org/cgi-bin/cvename.cgi?name=CVE-2020-8559) described in [ Security bulletins ](https://cloud.google.com/anthos/gke/docs/on-prem/security-bulletins?hl=ko#gcp-2020-009) . 
...
GKE on-prem ](https://cloud.google.com/anthos/gke/docs/on-prem/how-
to/upgrading?hl=ko) . GKE on-prem 1.4.0-gke.13 clusters run on Kubernetes
1.16.8-gke.6.
...
**Updated to Kubernetes 1.16:**
  ...
  * Please note that Kubernetes 1.16 has deprecated some of its APIs. For more information, see [ Kubernetes 1.16 deprecated APIs ](https://cloud.google.com/kubernetes-engine/docs/deprecations/apis-1-16?hl=ko) . 
...
**Simplified upgrade:**
  ...
  * This release provides a simplified upgrade experience via the following changes: 
    ...
    * Support skip version upgrade to enable users to upgrade the cluster from any patch release of a minor release to any patch release of the next minor release. For more information about the detailed upgrade procedure and limitations, see [ upgrading GKE on-prem ](https://cloud.google.com/anthos/gke/docs/on-prem/how-to/upgrading?hl=ko) . 
    * The [ alternate upgrade scenario for Common Vulnerabilities and Exposures ](https://cloud.google.com/anthos/gke/docs/on-prem/how-to/upgrading?hl=ko) has been deprecated. All upgrades starting with version 1.3.2 need to upgrade the entire admin workstation. 
...
**Improved installation and cluster configuration:**
  ...
  * The user cluster [ node pools ](https://cloud.google.com/anthos/gke/docs/on-prem/how-to/user-cluster-basic?hl=ko#nodepoolsname) feature is now generally available. 
...
**Improved disaster recovery capabilities:**
  ...
  * This release provides enhanced disaster recovery functionality to [ support backup and restore ](https://cloud.google.com/anthos/gke/docs/on-prem/how-to/backing-up?hl=ko#user_cluster_backups) HA user cluster with etcd. 
...
**Enhanced monitoring with Cloud Monitoring (formerly Stackdriver):**
  ...
  * This release provides better product monitoring and resource usage management via the following changes: 
    ...
    * Introduces a [ default monitoring dashboard ](https://cloud.google.com/anthos/gke/docs/on-prem/how-to/logging-and-monitoring?hl=ko#dashboards) . 
...
**Improved scale:**
  ...
  * This release improves the cluster scalability by supporting [ a maximum of 10 instead of 5 user clusters for each admin cluster ](https://cloud.google.com/anthos/gke/docs/on-prem/quotas?hl=ko) . 
...
checks?hl=ko) with any GKE on-prem [ download image
](https://cloud.google.com/anthos/gke/docs/on-prem/downloads?hl=ko#bundle-
latest) .
...
prem/how-to/upgrading?hl=ko#alternate_upgrade_scenario) , a timeout might
...
](https://cloud.google.com/anthos/gke/docs/on-prem/how-to/oidc?hl=ko) has been
...
](https://cloud.google.com/anthos/gke/docs/on-prem/quotas?hl=ko) .
...
ips?hl=ko#bundlepath) | [ DHCP ](http://cloud.google.com/anthos/gke/docs/on-
prem/how-to/install-dhcp?hl=ko#bundlepath) ), you must append the ` --fast `
...
prem/reference/gkectl/diagnose?hl=ko) command might fail if your clusters:
...
to/admin-workstation?hl=ko#copy_terraform) . vSphere provider version 1.16
...
prem/reference/config?hl=ko) to say that the minimum number of worker nodes is
three.
...
prem/how-to/preflight-checks?hl=ko#list) that validates an NTP server is
available.
...
to/preflight-checks?hl=ko#known_issue) from 1.2.0-gke.6 that prevented `
...
prem/release-notes?hl=ko#January_28_2020) from 1.2.0-gke.6 that affects
...
prem/docs/release-notes?hl=ko#september_26_2019) , the ` gkeconnect.proxy `
field is no longer in the GKE on-prem [ configuration file
](https://cloud.google.com/gke-on-prem/docs/reference/config?hl=ko) .
...
checks?hl=ko) command can fail with this error:
...
prem/docs/concepts/networking?hl=ko#support_for_vmware_nsx-t) .
...
to/preflight-checks?hl=ko) for validating your GKE on-prem configuration file
...
prem/docs/how-to/install-overview-basic?hl=ko) workflow. This workflow offers
...
](https://cloud.google.com/gke-on-prem/docs/how-to/install-csi-driver?hl=ko)
...
](https://cloud.google.com/gke-on-prem/docs/how-to/oidc?hl=ko) with the Anthos
...
](https://cloud.google.com/gke-on-prem/docs/downloads?hl=ko#latest) . Then,
see [ Upgrading admin workstation ](https://cloud.google.com/gke-on-
prem/docs/how-to/upgrading?hl=ko) and [ Upgrading clusters
](https://cloud.google.com/gke-on-prem/docs/how-to/upgrading?hl=ko) .
...
prem/docs/how-to/security/hardening-your-cluster?hl=ko) .

**FEATURE:**

Published [ Managing clusters ](https://cloud.google.com/gke-on-prem/docs/how-
to/administration/managing-clusters?hl=ko) .
...
clusters?hl=ko#admin_datadisk_folder) and installation topics.
...
admin-workstation?hl=ko) . Then, you should [ upgrade your clusters
](https://cloud.google.com/gke-on-prem/docs/how-to/administration/upgrading-
clusters?hl=ko) .
...
](https://cloud.google.com/gke-on-prem/docs/support/toolbox?hl=ko) .
...
bulletins?hl=ko#october-16-2019) .
...
feature—perform the following steps _before_ you upgrade your user clusters:
  ...
  2. Under the ` usercluster ` specification, add the ` antiaffinitygroups ` field as described in the [ ` antiaffinitygroups ` documentation ](https://cloud.google.com/gke-on-prem/docs/how-to/installation/install?hl=ko#antiaffinitygroups) : ` usercluster: ... antiaffinitygroups: enabled: false `
...
prem/docs/how-to/administration/default-storage-class?hl=ko) for your
clusters.
...
user-cluster?hl=ko#delete_unhealthy_cluster) with ` gkectl delete cluster
--force `

**FEATURE:**

You can now [ diagnose node issues ](https://cloud.google.com/gke-on-
prem/docs/support/debug-toolbox?hl=ko) using the ` debug-toolbox ` container
image.

**FEATURE:**

You can now [ skip validatations ](https://cloud.google.com/gke-on-
prem/docs/how-to/installation/install?hl=ko#skip_validate) run by ` gkectl `
commands.
...
prem/docs/how-to/installation/install?hl=ko#antiaffinitygroups) .
...
prem/docs/support/diagnose?hl=ko#diagnose_snapshot) .
...
prem/docs/how-to/installation/install?hl=ko) .
...
to/administration/deleting-a-user-cluster?hl=ko) .
...
prem/docs/security-bulletins?hl=ko#august-22-2019) .

**FIXED:**

[ Fixed a vulnerability in RBAC proxy ](https://cloud.google.com/gke-on-
prem/docs/security-bulletins?hl=ko#august-23-2019) .
...
prem/docs/reference/cheatsheet?hl=ko) .
...
to/upgrading/from-beta?hl=ko) , and review the following points:
...
lb?hl=ko) to configure a L4 load balancer. You can still choose to use the
...
](https://cloud.google.com/gke-on-prem/docs/overview?hl=ko#config) .
...
to/installation/install?hl=ko#generate_config) .
...
to/installation/install?hl=ko#validate_config) .
...
prem/docs/how-to/installation/install?hl=ko#prepare) .
...
to/administration/upgrading-clusters?hl=ko) to this version. You can no longer
...
clusters?hl=ko) can cause disruption or downtime for workloads that use [
...
to/installation/manual-lb?hl=ko) to GKE On-Prem version 1.0.10. To upgrade and
...
prem/docs/beta-1.4/how-to/administration/diagnose?hl=ko#capture_admin)
...
engine/connect/updating-agent?hl=ko) :
...
prem/docs/beta-1.4/how-to/administration/upgrading-a-cluster?hl=ko) :
...
to/administration/diagnose?hl=ko#performing_a_dry_run_for_a_snapshot) flag.

**FEATURE:**

The ` gkectl diagnose snapshot ` command now supports four [ scenarios
](https://cloud.google.com/gke-on-prem/docs/beta-1.3/how-
to/administration/diagnose?hl=ko#snapshot_scenarios) .
...
to/administration/upgrading-a-cluster?hl=ko#upgrading_the_ingress_controller)
...
prem/docs/beta-1.3/how-to/installation/install?hl=ko#enabling_ingress) .
...
](https://cloud.google.com/kubernetes-engine/connect/?hl=ko) .
...
to/installation/getting-started?hl=ko#download_ova) , a pre-configured virtual
...
to/administration/backing-up?hl=ko) .
...
adfs?hl=ko) and [ Authentication ](https://cloud.google.com/gke-on-
prem/docs/concepts/authentication?hl=ko) .
...
engine/connect/?hl=ko) has been migrated.
...
prem/docs/beta-1.0/how-to/administration/upgrading-a-cluster?hl=ko) to future
beta versions.

**FEATURE:**

[ Anthos Config Management ](https://cloud.google.com/anthos-config-
management/docs/?hl=ko) version 0.11.6 is now available.
...
to/installation/registering-a-user-cluster?hl=ko)

**CHANGED:**

[ Getting started ](https://cloud.google.com/gke-on-prem/docs/how-
to/installation/getting-started?hl=ko) has been updated with steps for
...
requirements ](https://cloud.google.com/gke-on-prem/docs/requirements?hl=ko) .
